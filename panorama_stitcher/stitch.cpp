...
vector<string> loadImagesFromFolder(const string &folder)
{
    vector<string> files;
...
int main(int argc, char *argv[])
{
    ...
    // Expand directories into image paths
    vector<String> expanded;
    for (const auto &p : img_names)
    {
        if (fs::is_directory(p))
        {
            auto imgs = loadImagesFromFolder(p);
            expanded.insert(expanded.end(), imgs.begin(), imgs.end());
        }
        else
        {
